...
Миграция выполнена. Далее итерация 11 (обслуживание редиректов) — она нужна, чтобы
старые URL реально открывались/301-или на новом сайте: middleware/route по таблице
Redirect + fallback по legacyId для `*/{id}-*.html`; скрипт check-links (уже в
package.json, файл scripts/check-links.ts написать), проверяющий, что ни один старый
URL из sitemap не отдаёт 404. Затем итерация 10 — SEO (метатеги, OG, JSON-LD, sitemap
по разделам, robots, RSS, крошки). Отдельно перед продом — чистка демо-материалов сида.
