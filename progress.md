...
9. [~] Скрипт миграции — scripts/migrate.ts + scripts/url-map.ts (карта старых
       ...
       ПРОВЕРЕНО на 5 статьях: тексты/обложки/фото/автор/дата/редиректы корректны,
       блок «похожие» правильно отсекается. НЕ выполнен полный прогон 555 статей.
...
Дождаться решения заказчика о полном прогоне миграции (555 статей, ~20-40 мин, грузит
живой esque.su). После прогона — итерация 10 (SEO: метатеги, OG, JSON-LD, sitemap по
разделам, robots, RSS, крошки) и итерация 11 (обслуживание редиректов: proxy/route по
таблице Redirect + fallback по legacyId для `*/{id}-*.html`; скрипт check-links,
проверяющий, что ни один старый URL из sitemap не отдаёт 404). npm-скрипт check-links
уже зарезервирован в package.json (файл scripts/check-links.ts — в итерации 11).
